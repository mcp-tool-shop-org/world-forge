...
`validateProject()` runs 89 structural checks using precomputed Map lookups for O(n) performance. Returns `{ valid, errors, errorCount, warningCount, schemaVersion }` plus optional `diagnostics` when `verbose` is set. `errorCount` is the canonical cardinality; `warningCount` is a deprecated alias of `errorCount` (it is not a separate advisory count — see `advisoryValidation()` for suggestions). `schemaVersion` is **always** stamped, including the structural-guard early return, via `finishResult`. New code should read `errorCount` and `schemaVersion`. An optional `ValidateOptions` parameter supports `verbose` mode for detailed output.

Before any rule runs, a structural guard confirms every required top-level array is actually an array. A truncated or corrupted import used to sail past this and fail later in a converter; now it fails immediately, with the field named. Every optional WorldProject array is guarded the same way when present — `buildings`, `hubs`, `strongholds`, `lootTables`, `transitions`, `strata`, `stratumLinks`, `hazardDefinitions`, and `projectTags` — absent stays valid, which is what keeps projects authored before those fields were added opening unchanged. `lootTables={}` / `strata=123` / `projectTags=null` therefore fail the same guard as a non-array `buildings`.
