...
3. **Pinte zonas** — arrastre en el lienzo para crear zonas, conéctelas y asígneles distritos.
...
Tipos y validaciones de TypeScript principales para la creación de mundos.
...
Aplicación web React 19 + Vite con gestión de estado Zustand, deshacer/rehacer con etiquetas de acción, autoguardado (con un intervalo de 30 segundos y un historial de 3 versiones), cambio de tema (claro/oscuro) y protección contra cambios no guardados.
...
| Objetos | Árbol jerárquico: distritos → zonas → entidades/puntos de interés/puntos de aparición. |
...
| Preajustes | Explorador de preajustes de regiones y encuentros con opciones de combinación/reemplazo. |
| Recursos | Biblioteca de recursos con búsqueda filtrada por tipo, detección de recursos huérfanos y paquetes de recursos. |
| Problemas | Validación en vivo y agrupada con navegación por clics para enfocar. |
| Dependencias | Escáner de dependencias con botones de reparación integrados. |
| Revisión | Panel de estado, vista general del contenido, exportación de resumen. |
| Guía | Lista de verificación para la primera ejecución con referencia de atajos de teclado. |

#### Lienzo y edición

- **Herramientas** — seleccionar, pintar zonas, conectar, colocar entidades, punto de interés, punto de aparición.
...
- **Vista previa de conexión** — línea discontinua cyan durante la colocación con la herramienta de conexión.
- **Mini mapa** — vista general de 200x150 (esquina inferior derecha), hacer clic para ir a ella.
- **Recorte de la vista** — solo renderiza objetos dentro de los límites visibles (margen de 64px).
...
- **Capas** — 7 opciones de visibilidad (cuadrícula, conexiones, entidades, puntos de interés, puntos de aparición, trasfondos, ambiente).
...
- **Proyectos agrupados:** Archivos `.wfproject.json` portátiles con metadatos de origen e información de dependencias.
- **Kits agrupados:** Exportación/importación de archivos `.wfkit.json` con validación, manejo de colisiones y seguimiento del origen.
- **Importación:** Detección automática de 4 formatos con informes estructurados de fidelidad.
- **Diferencias (Diff):** Seguimiento de cambios semánticos desde la importación.
- **Vista previa de la escena:** Composición HTML/CSS integrada de todas las vinculaciones visuales de las zonas.

## Modos de creación

World Forge separa el **género** (fantasía, cyberpunk, pirata) del **modo** (mazmorra, océano, espacio). El género es el estilo; el modo es la escala. El modo controla los valores predeterminados de la cuadrícula, el vocabulario de conexiones, las sugerencias de validación, la redacción de las guías y el filtrado de los ajustes predefinidos.
...
- 12 tipos de conexión (pasaje, puerta, escaleras, carretera, portal, secreto, peligro, canal, ruta, amarre, salto, sendero) con estilos visuales distintos, enrutamiento anclado al borde, flechas direccionales y estilo de guiones condicional.
...
- Colocaciones de entidades con estadísticas, recursos, perfiles de IA y metadatos personalizados.
- Colocaciones de objetos con ranura, rareza, modificadores de estadísticas y verbos concedidos.
...
- Manifiesto de activos (retratos, sprites, fondos, iconos, conjuntos de mosaicos) con vinculaciones específicas del tipo.
- Paquetes de activos (agrupaciones nombradas y versionadas con metadatos de compatibilidad, tema, licencia).
- Vista previa de la escena (composición integrada de todas las vinculaciones visuales de las zonas con detección de activos faltantes).

### Flujo de trabajo

- Presets de región (9 integrados, filtrados por modo) y presets de encuentros (10 integrados) con aplicación de combinación/sobrescritura y gestión CRUD de presets personalizados.
...
- Combinación de zonas y colocación por lotes de entidades (patrones de cuadrícula/aleatorio/círculo).
